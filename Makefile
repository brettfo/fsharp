...
proto: tools
	...
	$(DotNetExe) build src/fsharp/fsc/fsc.fsproj -f net5.0 -c Proto
...
build: proto restore
	...
	$(DotNetExe) build -c $(Configuration) -f net5.0 src/fsharp/fsc/fsc.fsproj
	...
	$(DotNetExe) build -c $(Configuration) -f net5.0 src/fsharp/fsi/fsi.fsproj
	$(DotNetExe) build -c $(Configuration) -f net5.0 tests/FSharp.Core.UnitTests/FSharp.Core.UnitTests.fsproj
	$(DotNetExe) build -c $(Configuration) -f net5.0 tests/FSharp.Build.UnitTests/FSharp.Build.UnitTests.fsproj

test: build
	$(DotNetExe) test -f net5.0 -c $(Configuration) --no-restore --no-build tests/FSharp.Core.UnitTests/FSharp.Core.UnitTests.fsproj -l "trx;LogFileName=$(CURDIR)/tests/TestResults/FSharp.Core.UnitTests.coreclr.trx"
	$(DotNetExe) test -f net5.0 -c $(Configuration) --no-restore --no-build tests/FSharp.Build.UnitTests/FSharp.Build.UnitTests.fsproj -l "trx;LogFileName=$(CURDIR)/tests/TestResults/FSharp.Build.UnitTests.coreclr.trx"
